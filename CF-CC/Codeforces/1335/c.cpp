...
int main() {
    ...
    while (tc--) {
        auto Solve = [&]() {
            ...
            vector <ii> p;
            for (auto x : s) {
                p.emplace_back(x, freq[x]);
            }
            sort(p.begin(), p.end(), [](const ii &a, const ii &b) -> bool {
                return a.second > b.second;
            });
            int ateam = p[0].second;
            int bteam = (int)p.size() - 1;
            if (ateam > bteam) bteam++, ateam--;
            cout << min(ateam, bteam) << endl;
        };
        Solve();
    }
    return 0;
