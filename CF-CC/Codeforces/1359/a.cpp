    ...
    while (tc--) {
        auto Solve = [&]() {
            ...
            if (no_of_cards >= m) {
                ...
                int winning_player = no_of_cards;
                m -= no_of_cards;
                int rest_of_the_player = k - 1;
                int mx;
                if (rest_of_the_player >= m) {
                    mx = 1;
                } else {
                    mx = (m % rest_of_the_player == 0) ? (m / rest_of_the_player) : (m / rest_of_the_player + 1);
                }
                cout << winning_player - mx << endl;
            }
        };
        Solve();
    }
