...
int main() {
    ...
    auto Solve = [&]() {
        ...
        int n, t;
        cin >> n >> t;
        vector <int> books(n + 1), cumu_books(n + 1, 0);
        for (int i = 1; i <= n; ++i) {
            cin >> books[i];
        }
        for (int i = 1; i <= n; ++i) {
            cumu_books[i] = cumu_books[i - 1] + books[i];
        }
        function <int(int)> bs = [&](int extra) {
            int lo = 1, hi = n;
            while (lo <= hi) {
                int mid = lo + (hi - lo) / 2;
                int check = cumu_books[mid];
                if (check > extra) {
                    hi = mid - 1;
                } else if (check <= extra) {
                    lo = mid + 1;
        ...
        int taken_so_far = 0, books_cnt = 0, answer = 0, initial = 1;
        for (int i = 1; i <= n; ++i) {
            if (taken_so_far + books[i] <= t) {
                taken_so_far += books[i];
                ++books_cnt;
            } else {
                answer = max(answer, books_cnt);
                int extra = cumu_books[i] - t;
                int in = bs(extra);
                books_cnt = i - in;
                taken_so_far = cumu_books[i] - cumu_books[in];
            }
        }
        answer = max(answer, books_cnt);
        cout << answer << endl;
