...
int main() {
    ...
    while (tc--) {
        auto Solve = [&]() {
                ...
                cout << "Running\n";
            #endif
            vector <int> weekdays_work(5);
            for (int i = 0; i < 5; ++i) {
                cin >> weekdays_work[i];
            }
            int p;
            cin >> p;
            int total_work_need_to_be_done = 0;
            int total_hours = 24*5;
            for (int i = 0; i < 5; ++i) {
                total_work_need_to_be_done += (weekdays_work[i] * p);
            }
            if (total_hours < total_work_need_to_be_done) {
                cout << "Yes\n";
            } else {
                cout << "No\n";
            }
